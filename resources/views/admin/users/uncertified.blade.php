...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$(document).on('click', '.checki', function(){
...
<div class="info">
    ...
    <div class="basic">
		...
		<div class="basicform">
			<p class="allsignin">{{ trans('admin.all_uncertified') }}</p>
			<p class="editl">Edit</p>
		</div>
			...
			<nav aria-label="..." class="paginate">
			  <ul class="pagination">
			    <li class="page-item {{ $page > 1 ? '' : 'disabled' }}">
			      <a class="page-link" href="{{ $page > 1 ? url('admin/uncertified') . '?page=' . ($page - 1) : '#' }}">Previous</a>
			    </li>
			    @for($i = 1; $i <= $pagesNumber; $i++)
			    <li class="page-item {{ $page == $i ? 'active' : '' }}">
			      <a class="page-link" href="{{ url('admin/uncertified') }}?page={{ $i }}">{{ $i }} <span class="sr-only">{{ $page == $i ? '(current)' : '' }}</span></a>
			    </li>
			    @endfor
			    <li class="page-item {{ $page == $pagesNumber ? 'disabled' : '' }}">
			      <a class="page-link" href="{{ $page == $pagesNumber ? '#' : url('admin/uncertified') . '?page=' . ($page + 1) }}">Next</a>
			    </li>
			  </ul>
			</nav>
		@endif
